...
public class TradeServiceImpl implements TradeService {
    ...
    public Long createTrade(Long userId, Long payerId, Long payeeId, Long postId) {
        ...
        if (payerId == null || payeeId == null || postId == null) {
            throw new TradeServiceException(MessageConstant.ALL_FIELDS_REQUIRED);
        }

        // 创建交易的用户应为付款方，且付款方与收款方不能为同一人
        if (!Objects.equals(userId, payerId) || payerId.equals(payeeId)) {
            throw new TradeServiceException(MessageConstant.TRADE_ERROR);
        }
        ...
        if (payer == null || payee == null || payer.getIsDeleted() == 1 || payee.getIsDeleted() == 1) {
            throw new TradeServiceException(MessageConstant.TRADING_USER_NOT_EXISTS);
        }

        // 检查帖子是否存在
        Post post = postMapper.getByPostId(postId);
        if (post == null || post.getIsDeleted() == 1) {
            throw new TradeServiceException(MessageConstant.POST_NOT_EXISTS);
        }

        // 检查帖子是否已经被交易
        if (Objects.equals(post.getStatus(), PostConstant.STATUS_HAS_TRADED)) {
            throw new TradeServiceException(MessageConstant.ITEM_HAS_BEEN_TRADED);
        }
        ...
        if (trade != null && trade.getStatus() != TradeConstant.STATUS_FAILURE) {
            throw new TradeServiceException(MessageConstant.TRADE_EXISTS);
        }
    ...
    public void deleteTrade(Long userId, Long tradeId) {
        ...
        if (trade == null) {
            throw new TradeServiceException(MessageConstant.TRADE_NOT_EXISTS);
        }

        // 交易进行中将无法删除交易记录
        Integer status = trade.getStatus();
        if (status == TradeConstant.STATUS_IN_PROGRESS || status == TradeConstant.STATUS_ESTABLISHED) {
            throw new TradeServiceException(MessageConstant.TRADE_IN_PROGRESS);
        }
        ...
        if (Objects.equals(userId, payerId)) {
            ...
            throw new TradeServiceException(MessageConstant.TRADE_OWNER_MISMATCH_ERROR);
        }
    ...
    public void cancelTrade(Long userId, Long tradeId){
        ...
        if (payeeDeleted == 1 || payerDeleted == 1) {
            throw new TradeServiceException(MessageConstant.TRADE_DELETED_OVER);
        }
        ...
        if(!Objects.equals(userId, payerId) && !Objects.equals(userId, payeeId)){
            throw new TradeServiceException(MessageConstant.TRADE_PAY_MISMATCH_ERROR);
        }

        // 检查交易是否结束（完成或失败）
        Integer status = trade.getStatus();
        if (status == TradeConstant.STATUS_SUCCESS || status == TradeConstant.STATUS_FAILURE) {
            throw new TradeServiceException(MessageConstant.TRADE_OVERED);
        }
    ...
    public void establishTrade(Long userId, Long tradeId) {
        ...
        if(!Objects.equals(userId, payeeId)) {
            throw new TradeServiceException(MessageConstant.TRADE_PAYEE_MISMATCH_ERROR);
        }

        // 检查交易是否已建立
        Integer status = trade.getStatus();
        if (status == TradeConstant.STATUS_ESTABLISHED) {
            throw new TradeServiceException(MessageConstant.TRADE_ESTABLISH);
        }

        // 检查交易是否结束（完成或失败）
        if (status == TradeConstant.STATUS_SUCCESS || status == TradeConstant.STATUS_FAILURE) {
            throw new TradeServiceException(MessageConstant.TRADE_OVER);
        }
    ...
    public void successTrade(Long userId, Long tradeId) {
        ...
        Long payerId = trade.getPayerId();
        if(!Objects.equals(userId, payerId)) {
            throw new TradeServiceException(MessageConstant.TRADE_PAYER_MISMATCH_ERROR);
        }

        // 检查交易是否已建立
        Integer status = trade.getStatus();
        if (status == TradeConstant.STATUS_IN_PROGRESS) {
            throw new TradeServiceException(MessageConstant.TRADE_NOY_ESTABLISH);
        }
    ...
    public PageResult tradePageQueryByUserId(Long userId, Integer pageNum, Integer pageSize) {
        ...
        Page<TradeQueryVO> page = tradeMapper.pageQueryByUserId(userId);

        // 查询失败则抛出异常
        if (page == null) {
            throw new TradeServiceException(MessageConstant.PAGE_QUERY_ERROR);
        }
